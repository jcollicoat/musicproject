...
export async function GET(request: NextRequest) {
    try {
        ...
        const endpoint = createIdApiEndpoint(request.nextUrl.pathname, spotifyEndpoint);
        const spotifyAudioFeatures = await axios.get<SpotifyAudioFeatures>(endpoint, {
            headers: {
                Authorization: accessToken
            }
        })
        return NextResponse.json(buildSpotifyAudioFeatures(spotifyAudioFeatures.data))
    }catch (error) {
